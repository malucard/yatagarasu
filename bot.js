	...
	if(roles.length === players.length) {
		...
		for(let i = 0; i < roles.length; i++) {
			if(roles[i][2] === "m") {
				...
				(new Discord.Channel()).pe(players[i], )
...
If you're a power role, please send your actions to the game master, not to me!");
		...
		receivedMessage.member.send("**Full Description**" + village + mafia + third);
...
client.on("ready", () => {
	client.user.setActivity(":spoilerhelp", {type: "LISTENING"});
...
client.on("message", async receivedMessage => {
	if(receivedMessage.author !== client.user) {
		if(receivedMessage.channel.name === "mafia-general" || receivedMessage.channel.name === "mafia-lobby" || receivedMessage.channel.name === "bot-test-channel") {
			...
			if(receivedMessage.content === ":startmafia" && receivedMessage.member.roles.has(master.id)) {
				stopped = false;
				receivedMessage.react("497430068331544577");
			} else if(receivedMessage.content === ":stopmafia" && receivedMessage.member.roles.has(master.id)) {
				stopped = true;
				receivedMessage.react("497430068331544577");
			} else if(!stopped) {
...
client.login(process.env.keebot_secret_token);
