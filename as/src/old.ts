...
class Role {
	...
	macho?: boolean = false;
	beginGame: (member: Discord.GuildMember, player: Player) => void;
	endGame: (member: Discord.GuildMember, player: Player) => void;
	beginNight: (member: Discord.GuildMember, player: Player) => void;
	endNight: (member: Discord.GuildMember, player: Player) => void;
	die: (member: Discord.GuildMember, player: Player) => void;
}

function getSide(player: Player): Side {
...
function isSide(player: Player, side: Side) {
	return getSide(player) == side;
}

let roles: {[name: string]: Role} = {
	Blue: {
		name: "Blue",
		side: Side.VILLAGE,
		beginGame: (member: Discord.GuildMember, player: Player) => {
			member.send("You are a Blue, number " + player.number + ".");
		},
		endGame: (member: Discord.GuildMember, player: Player) => {},
		beginNight: (member: Discord.GuildMember, player: Player) => {
			player.actionDone = true;
		},
		endNight: (member: Discord.GuildMember, player: Player) => {},
		die: (member: Discord.GuildMember, player: Player) => {}
	},
	...
	Oracle: {
		...
		beginNight: (member: Discord.GuildMember, player: Player) => {
			...
			member.send("Night " + day + " has begun. React with the number of who you wish to visit." + text).then(async (message: Discord.Message) => {
				await message.react("❌");
				for(let player of players) {
					if(player.id !== member.id) {
				...
				let collector = message.createReactionCollector((reaction, user: Discord.User) => user.id === member.id);
	...
	Doc: {
		...
		beginNight: (member: Discord.GuildMember, player: Player) => {
			...
			member.send("Night " + day + " has begun. React with the number of who you wish to save." + text).then(async (message: Discord.Message) => {
				await message.react("❌");
				for(let player of players) {
					if(player.id !== member.id) {
						await message.react(player.number + "\u20e3");
	...
	Cop: {
		...
		beginNight: (member: Discord.GuildMember, player: Player) => {
			...
			member.send("Night " + day + " has begun. React with the number of who you wish to investigate." + text).then(async (message: Discord.Message) => {
				let collector = message.createReactionCollector((reaction, user: Discord.User) => user.id === member.id);
				collector.on("collect", (reaction) => {
					if(reaction.emoji.name === "❌") {
						player.data = null;
	...
	TalentScout: {
		...
		beginNight: (member: Discord.GuildMember, player: Player) => {
			...
			member.send("Night " + day + " has begun. React with the number of who you wish to scout." + text).then(async (message: Discord.Message) => {
				let collector = message.createReactionCollector((reaction, user: Discord.User) => user.id === member.id);
				collector.on("collect", (reaction) => {
					if(reaction.emoji.name === "❌") {
						player.data = null;
	...
	Gunsmith: {
		...
		beginNight: (member: Discord.GuildMember, player: Player) => {
			...
			member.send("Night " + day + " has begun. React with the number of who you wish to give a gun to." + text).then(async (message: Discord.Message) => {
				let collector = message.createReactionCollector((reaction, user: Discord.User) => user.id === member.id);
				collector.on("collect", (reaction) => {
					if(reaction.emoji.name === "❌") {
						player.data = null;
	...
	Vanilla: {
		...
		side: Side.MAFIA,
		beginGame: (member: Discord.GuildMember, player: Player) => {
			let secret = member.guild.channels.find((x) => x.name === "mafia-secret-chat") as Discord.TextChannel;
...
function calculateLynch(): string {
...
function beginNight(channel: Discord.TextChannel, mafiaPlayer: Discord.Role) {
	...
	let secret = channel.guild.channels.find((x) => x.name === "mafia-secret-chat") as Discord.TextChannel;
	let text = "";
	if(day === 1) {
		text = " 1-" + players.length;
	} else {
...
const setups: {[name: string]: Setup} = {
